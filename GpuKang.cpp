...
bool RCGpuKang::Prepare(EcPoint _PntToSolve, int _Range, int _DP, EcJMP* _EcJumps1, EcJMP* _EcJumps2, EcJMP* _EcJumps3)
{
	...
	snprintf(ProgressFileName, sizeof(ProgressFileName), "PROGRESS/PROGRESS.GPU.%s.%s.BIN", gpuId.c_str(), gParamsHash4.c_str());
...
bool RCGpuKang::LoadProgress()
{
	...
	if (memcmp(hdr.magic, "RCKANGPROGv2", 12) != 0 || hdr.version != 2 || hdr.range != (u32)Range || hdr.dp != (u32)DP || hdr.kangCnt != (u32)KangCnt)
	{
		fclose(f);
		return false;
	}
